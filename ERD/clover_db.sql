...
FROM userdto ut, write_table wt WHERE ut.user_uid = wt.user_uid AND wt.wr_uid = 15;

SELECT wt.wr_uid, ut.user_name, wt.wr_category, wt.wr_subject, wt.wr_content, wt.wr_longitude, wt.wr_latitude 
FROM userdto ut, write_table wt WHERE wt.user_uid = ut.user_uid ORDER BY wr_uid desc;

INSERT INTO write_table(wr_category, wr_subject, wr_content, wr_latitude, wr_longitude, user_uid) 
		VALUES('부모님', '찾아주세요', '네',  37.123125, 127.123124, (SELECT user_uid FROM userdto WHERE user_id='katie'));
SELECT user_uid FROM userdto WHERE user_id='katie'
...
VALUES (127.123124, 37.123125, '여기서 찾았어요', 42, (SELECT user_uid FROM userdto WHERE user_id = 'katie'));
...
INSERT INTO QR_Table
		...
		VALUES
			('신현담', '부모님', 30, '서울', '01062423233', '찾아주세요',  (SELECT user_uid FROM userdto WHERE user_id = 'shdking'));
SELECT user_uid FROM userdto WHERE user_id = 'shdking';
INSERT INTO QR_Table
			(qr_name, qr_category, qr_age, qr_address, qr_phone, qr_content, user_uid)
		VALUES
			('신현담', '부모님', 30, '서울', '01062423233', '찾아주세요',  (SELECT user_uid FROM userdto WHERE user_id = 'shdking'));
