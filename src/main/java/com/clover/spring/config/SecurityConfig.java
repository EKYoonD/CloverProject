...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests()
				...
				// ↓ /sample/member/** 주소로 들어오는 요청은 '인증' 뿐 아니라 ROLE_MEMBER 나 ROLE_ADMIN 권한을 갖고
				// 있어야 한다 ('인가')
				.antMatchers("/clover/member/**")
				.access("hasRole('ROLE_MEMBER') or hasRole('ROLE_ADMIN') or hasRole('ROLE_USER')")
				...
				.and()
				// 로그인 처리
				.formLogin().loginPage("/login2").usernameParameter("userid") // 만약 로그인 username 이 name="username" 이
																				// 아닌경우
