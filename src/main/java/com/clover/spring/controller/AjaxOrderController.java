...
import com.clover.spring.domain.AjaxOrderList;
...
public class AjaxOrderController {
	...
	public AjaxOrderList list(
		...
		try {			
			// 글 전체 개수 구하기
			totalCnt = ajaxOrderService.count();
			...
			list = ajaxOrderService.list(from, pageRows);
		...
		AjaxOrderList result = new AjaxOrderList();
	...
	public AjaxOrderList view(@PathVariable int uid) {
		List<KakaoPayDTO> list = null;
		...
		try {			
			list = ajaxOrderService.viewByUid(uid);  // 조회수 증가 + 읽기
